...
###################################################
# Goals: 
# Create new table for different keywords and 
# send the table to database.
# Use if_exist = "replace"
# Procedure:
# 1. search for data
# 2. add the table to database
###################################################
...
if __name__ == "__main__": 
    ...
    # delay_time, search, filename (if already get some data but not finished)
    # py -3.10 anue_past_news.py 3 3443 2023121718_anue_3443.json
    # store in db
    ##################################################    
    if (len(sys.argv) < 2):
    ...
    if sys.argv[1]:
        ...
        to_database(filename=filename, search=search)
