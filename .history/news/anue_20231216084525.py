...
def onStart(path, search):
    suffix_output = f"{search}.json"
...
def get_item(path):
    """
    file_path = 'C:/Users/USER/Desktop/sentiment_Text/news/texts/2023121521_友達.json'
    """
    dict_posts = {}
    # Open the file and load the JSON data
    with open(path, 'r', encoding="utf-8") as file:
        for i in file:
            unitpost = json.loads(i)
            dict_posts = {**dict_posts, **unitpost}
    return dict_posts
...
def main(delay_time, search):
    ...
    # Use url as key
    path = 'C:/Users/USER/Desktop/sentiment_Text/news/texts/2023121521_anue_友達.json'
    if Path(path).exists():
        ...
        try:
            ...
            for article_link in new_article:
                try:
                    ...
                    dict_posts[article_link] = content
                    item[article_link] = [time, content]
                    print(item)
                    print('-'*80)
                    process_item(data_json, item)
                except:
                    print(f"Get no texts from the url: {article_link}!!!!!")
                

            time = datetime.datetime.strptime(time[:10], "%Y/%m/%d %H:%M")
            if time <= stop_date:
                break
